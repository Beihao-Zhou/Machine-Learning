...
h = sigmoid( a2*Theta2');
y_choice = eye(num_labels);
y = y_choice(y,:);

J = sum(-y.*log(h)-(1-y).*log(1-h),'all')/m + lambda/2/m*(sum(Theta1(:,2:end).^2,'all')+sum(Theta2(:,2:end).^2,'all'));
...
for i = 1:m
    delta3(i,:) = h(i,:) - y(i,:);   %10
    Theta2_trans = Theta2';
    delta2(i,:) = (Theta2_trans(2:end,:)*delta3(i,:)'.*sigmoidGradient(z2(i,:)'))';   %25
